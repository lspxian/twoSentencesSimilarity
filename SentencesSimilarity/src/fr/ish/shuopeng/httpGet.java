package fr.ish.shuopeng;

import java.io.*;
import java.net.*;
import java.util.regex.Matcher;
import java.util.regex.Pattern;
public class httpGet {
	public static String getHTML(String urlToRead) {
	      URL url;
	      HttpURLConnection conn;
	      BufferedReader rd;
	      String line;
	      String result = "";
	      try {
	         url = new URL(urlToRead);
	         conn = (HttpURLConnection) url.openConnection();
	         conn.setRequestMethod("GET");
	         rd = new BufferedReader(new InputStreamReader(conn.getInputStream()));
	         while ((line = rd.readLine()) != null) {
	            result += line;
	         }
	         rd.close();
	      } catch (IOException e) {
	         e.printStackTrace();
	      } catch (Exception e) {
	         e.printStackTrace();
	      }
	      return result;
	   }
	
	public static double wordSimilarity(String w1, String w2){
		
		//probleme non et verb
		//TODO
		 //String url = "http://maraca.d.umn.edu/cgi-bin/similarity/similarity.cgi?word1=keep%23v&senses1=all&word2=use%23v&senses2=all&measure=wup&rootnode=yes";
		 String url = "http://maraca.d.umn.edu/cgi-bin/similarity/similarity.cgi?word1="+w1+"&senses1=all&word2="+w2+"&senses2=all&measure=wup&rootnode=yes";
	     String result = getHTML(url);
	     
	     Pattern pattern = Pattern.compile("using wup is (\\d+\\.\\d+)");
	     Matcher matcher = pattern.matcher(result);
	     
	     while(matcher.find()){
	    	 result = matcher.group();
	    	 System.out.println(matcher.group());
	    	 
	    	 pattern = Pattern.compile("(\\d+\\.\\d+)");
	    	 matcher = pattern.matcher(result);
	    	 
	    	 while(matcher.find()){
	    		 return Double.parseDouble(matcher.group());
	    	 }
	    	 
	     }
		
		return -1;
	}

	   public static void main(String args[])
	   {

	     
	   }
}
